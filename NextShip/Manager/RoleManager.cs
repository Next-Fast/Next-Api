...
public sealed class NextRoleManager : IRoleManager
{
    ...
    public T GetRole<T>() where T : IRole => (T)Roles.FirstOrDefault(n => n is T);
    
    public IEnumerable<T> GetRoles<T>() where T : IRole => Roles.FindAll(n => n is T).Select(n =>　(T)n);
    
    public T GetCreator<T>() where T : IRoleCreator => (T)RoleCreators.FirstOrDefault(n => n is T);
}