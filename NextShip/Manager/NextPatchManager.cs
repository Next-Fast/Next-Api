...
public class NextPatchManager : IPatchManager
{
    ...
    private List<Harmony> _HarmonyS = [];

    private HashSet<MethodInfo> _Patches = [];
    ...
    public void Register(Harmony harmony)
    {
        if (_HarmonyS.Exists(n => n.Id == harmony.Id))
        {
            ...
            old.Do(n =>
            {
                ...
                {
                    n.Unpatch(method, HarmonyPatchType.All);
                }
    ...
    public void SetRoot(Harmony harmony) => RootHarmony = harmony;
