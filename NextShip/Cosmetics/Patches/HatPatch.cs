...
{
    [HarmonyPatch(typeof(HatParent), nameof(HatParent.SetHat), typeof(int)), HarmonyPrefix]
    ...
    [HarmonyPatch(typeof(CosmeticsCache), nameof(CosmeticsCache.GetHat)), HarmonyPrefix]
    ...
    private static void PlayerPhysicsHandleAnimationPatch(PlayerPhysics __instance) {
        if (!CustomCosmeticsManager.AllCustomCosmeticNameAndInfo.ContainsKey(__instance.myPlayer.cosmetics.hat.Hat.name)) return;
