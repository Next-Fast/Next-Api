...
public class RPCInfoLog
{
    ...
    internal static class RPCInfoLogPatch
    {
        private static IEnumerable<Type> InnerNetObjectTypes { get; } = typeof(InnerNetObject).Assembly.GetTypes()
            .Where(x => x.IsSubclassOf(typeof(InnerNetObject))).ToList();
        ...
        public static void Prefix(InnerNetObject __instance, [HarmonyArgument(0)] byte callId,
            [HarmonyArgument(1)] MessageReader reader)
        {
            Info(
                $"RPCHandle {nameof(__instance)} CallId{callId} Length:{reader.Length} Pos:{reader.Position} Tag:{reader.Tag}",
                "RPCInfoLog");
        }
    }
}