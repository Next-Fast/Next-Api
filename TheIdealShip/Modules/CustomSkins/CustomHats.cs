...
namespace TheIdealShip.Modules
{
    ...
    public class CustomHats
    {
        ...
        private static List<CustomHat> createCustomHatDetails(string[] hats, bool fromDisk = false)
        {
            ...
            for (int i = 0; i < hats.Length; i++)
            {
                ...
                if (options.Contains("back") && options.Contains("flip")) backflips.Add(p[0], hats[i]);
                else
                if (options.Contains("climb")) climbs.Add(p[0], hats[i]);
                else
                if (options.Contains("back")) backs.Add(p[0], hats[i]);
                else
                if (options.Contains("flip")) flips.Add(p[0], hats[i]);
                else
                {
            ...
            foreach (string k in fronts.Keys)
            {
                ...
                if (br != null) hat.backresource = br;

                if (cr != null) hat.climbresource = cr;

                if (fr != null) hat.flipresource = fr;

                if (bfr != null) hat.backflipresource = bfr;
        ...
        private static Sprite CreateHatSprite(string patch, bool fromDisk = false)
        {
            Texture2D texture = fromDisk ? Helpers.LoadTextureFromDisk(patch) : Helpers.LoadTextureFromResources(patch);
        ...
        private static HatData CreateHatBehaviour(CustomHat ch, bool fromDisk = false, bool textOnly = false)
        {
            if (hatShader == null)
            {
            ...
            if (ch.climbresource != null) hat.hatViewData.viewData.ClimbImage = CreateHatSprite(ch.climbresource, fromDisk);
            ...
            hat.ChipOffset = new Vector2(0, 0.2f);
            hat.Free = true;

            if (ch.adaptive && hatShader != null) hat.hatViewData.viewData.AltShader = hatShader;
            ...
            if (ch.flipresource != null) extend.FlipImage = CreateHatSprite(ch.flipresource, fromDisk);

            if (ch.backflipresource != null) extend.BackFlipImage = CreateHatSprite(ch.backflipresource, fromDisk);
        ...
        {
            string filePatch = Path.GetDirectoryName(Application.dataPath) + @"\Costume\Hats";
            chd.resource = filePatch + chd.resource;
            if (chd.backresource != null) chd.backresource = filePatch + chd.backresource;

            if (chd.climbresource != null) chd.climbresource = filePatch + chd.climbresource;

            if (chd.flipresource != null) chd.flipresource = filePatch + chd.flipresource;

            if (chd.backflipresource != null) chd.backflipresource = filePatch + chd.backflipresource;
        ...
        private static class HatManagerPatch
        {
            ...
            static void Prefix(HatManager __instance)
            {
                ...
                {
                    if (!LOADED) Exception(ex, filename:"CustomHats");
                }
                LOADED = true;
            }
            ...
            private static class HatParentSetHatPatchColor
            {
                static void Prefix(HatParent __instance)
                {
                    if (DestroyableSingleton<TutorialManager>.InstanceExists)
                    {
                        try
                        {
                            string filePath = Path.GetDirectoryName(Application.dataPath) + @"\Costume\Hats";
    ...
    public class CustomHatLoader
    {
        ...
        public static string[] HatRepo = new string[]
        {
            "https://raw.githubusercontent.com/TheIdealShipAU/TheIdealShip-Hats/main",
            "https://raw.githubusercontent.com/Huier-Huang/TheOtherHats/master"
        };
        ...
        {
            if (res == null || !res.EndsWith(".png")) return null;
        ...
        public static async Task<HttpStatusCode> FetchHats(string repo)
        {
            ...
            try
            {
                ...
                {
                    Msg("目标没有内容", filename:"CustomHats");
                ...
                {
                    var hatFileResponse = await http.GetAsync($"{HatRepo}/hats/{file}", HttpCompletionOption.ResponseContentRead);
            ...
            {
                Exception(ex, filename:"CustomHats");
            }
